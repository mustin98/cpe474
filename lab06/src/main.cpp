...
void drawGL()
{
	...
	glUniform2fv(progTex.getUniform("cps"), 25, grid.getAllCPs()[0].data());
	...
	shape.draw(progTex.getAttribute("tileIndex"), progTex.getAttribute("vertLocalPos"), progTex.getAttribute("vertTex"));
	texture.unbind(0);
	progTex.unbind();
