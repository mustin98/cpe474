...
void ShapeObj::init()
{
	...
	glBufferData(GL_ARRAY_BUFFER, posBuf.size()*sizeof(float), &shapes[0].mesh.positions[0], GL_STATIC_DRAW);
	...
	if(!norBuf.empty()) {
		...
		glBufferData(GL_ARRAY_BUFFER, norBuf.size()*sizeof(float), &shapes[0].mesh.normals[0], GL_STATIC_DRAW);
	} else {
		norBufID = 0;
	}
